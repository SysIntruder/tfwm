...
void tfwm_layout_apply_tiling(void) {
    ...
    if (ws->window_len > 2) {
        slave_height =
            ((g_screen->height_in_pixels - TFWM_BAR_HEIGHT) / (ws->window_len - 1)) -
            (TFWM_BORDER_WIDTH * 2);
    }
